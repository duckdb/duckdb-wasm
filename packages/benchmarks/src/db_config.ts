...
export async function loadTPCHSQL(dataFetcher: (filename: string) => Promise<string>): Promise<void> {
    for (let i = 1; i <= 22; i++) {
...
export abstract class DuckDBSyncMaterializedProxy implements DBProxy {
    public name: string;
...
export abstract class DuckDBSyncStreamProxy extends DuckDBSyncMaterializedProxy {
    constructor(db: duckdb.DuckDBBindings) {
        super(db);
...
export abstract class DuckDBAsyncStreamProxy implements DBProxy {
    public name: string;
...
export class SQLjsProxy implements DBProxy {
    name: string;
    db: SQL.Database;
    private tpchRuns: Set<number>;
...
export class AlaSQLProxy implements DBProxy {
    name: string;
    private tpchRuns: Set<number>;
...
export class LovefieldProxy implements DBProxy {
    name: string;
    builder?: lf.Builder;
    ...
    async join(): Promise<number> {
        ...
        const builder = this.db!.select(lf.fn.count().as('cnt'))
            .from(a)
...
export class ArqueroProxy implements DBProxy {
    name: string;
    ...
    async join(): Promise<number> {
        return this.tables['orders'].join(this.tables['lineitem'], ['o_orderkey', 'l_orderkey']).count().objects()[0]
            .count;
    }
...
export class NanoSQLProxy implements DBProxy {
    name: string;
