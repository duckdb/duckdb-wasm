 ...
 *
 * Why this is needed:
 * DuckDB's file operations (openFile, readFile, etc.) are synchronous C++ calls that
 * cannot be made async. However, OPFS file creation requires async APIs. When DuckDB
 * needs to create temporary files for spilling data to disk, it calls openFile
 * synchronously, expecting an immediate file handle.
 *
 * The solution:
 * We pre-create a pool of OPFS files with their sync access handles during the async
 * temp directory registration. When DuckDB needs a temp file, we can synchronously
 * hand out one of these pre-created handles from the pool. When the file is closed,
 * we clear it and return it to the pool for reuse.
 *
 * This allows DuckDB's synchronous file operations to work with OPFS's async APIs.
 */
