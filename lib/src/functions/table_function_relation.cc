...
unique_ptr<TableRef> TableFunctionRelation::GetTableRef() {
    ...
    for (auto &[k, v] : named_parameters) {
        auto l = make_uniq<ColumnRefExpression>(k);
        auto r = make_uniq<ConstantExpression>(v);
        auto eq = make_uniq<ComparisonExpression>(ExpressionType::COMPARE_EQUAL, std::move(l), std::move(r));
