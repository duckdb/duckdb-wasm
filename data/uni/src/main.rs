...
impl Error {
    fn new(msg: String) -> Box<Self> {
        Box::new(Self{message: msg.to_string()})
    }
}
    ...
    match || -> Result<(), Box<dyn std::error::Error>> {
        ...
        while let Some(mut col_writer) = row_group_writer.next_column()? {
            if column_id >= data.len() {
                return Err(Error::new(format!("Schema contains more columns than provided. (expected {})", data.len())));
            }
            match (&data[column_id], &mut col_writer) {
                ...
                (_, _) => {
                    return Err(Error::new(format!("Type mismatch for column {}", column_id)));
                }
            }
...
fn main() {
    ...
    write_table(out_dir.join("professoren.parquet"), "
        message schema {
    ...
    ", vec![
        ...
        Column::Varchar(vec!["Sokrates", "Russel", "Kopernikus", "Popper", "Augustinus", "Curie", "Kant"]),
        Column::Varchar(vec!["C4", "C4", "C3", "C3", "C3", "C4", "C4"]),
        Column::Integer(vec![226, 232, 310, 52, 309, 36, 7]),
    ]);

    write_table(out_dir.join("studenten.parquet"), "
        message schema {
    ...
    ", vec![
        ...
        Column::Varchar(vec!["Xenokrates", "Jonas", "Fichte", "Aristoxenos", "Schopenhauer", "Carnap", "Theophrastos", "Feuerbach"]),
        Column::Integer(vec![18, 12, 10, 8, 6, 3, 2, 2]),
    ]);

    write_table(out_dir.join("vorlesungen.parquet"), "
        message schema {
    ...
    ", vec![
        Column::Integer(vec![5001, 5041, 5043, 5049, 4052, 5052, 5216, 5259, 5022, 4630]),
        Column::Varchar(vec!["Grundzüge", "Ethik", "Erkenntnistheorie", "Mäeutik", "Logik", "Wissenschaftstheorie", "Bioethik", "Der Wieer Kreis", "Glaube und Wissen", "Die 3 Kritiken"]),
        Column::Integer(vec![4, 4, 3, 2, 4, 3, 2, 2, 2, 4]),
        Column::Integer(vec![2137, 2125, 2126, 2125, 2125, 2126, 2126, 2133, 2134, 2137]),
    ]);

    write_table(out_dir.join("vorraussetzen.parquet"), "
        message schema {
    ...
    write_table(out_dir.join("hoeren.parquet"), "
        message schema {
            required int32 MatrNr;
            required int32 VorlNr;
        }
    ", vec![
        Column::Integer(vec![26120, 27550, 27550, 28106, 28106, 28106, 28106, 29120, 29120, 29120, 29555, 25403]),
        Column::Integer(vec![5001, 5001, 4052, 5041, 5052, 5216, 5259, 5001, 5041, 5049, 5022, 5022]),
    ]);

    write_table(out_dir.join("pruefen.parquet"), "
        message schema {
            required int32 MatrNr;
            required int32 VorlNr;
            required int32 PersNr;
    ...
    write_table(out_dir.join("assistenten.parquet"), "
        message schema {
            required int32 PersNr;
            required byte_array Name (utf8);
    ...
    ", vec![
        ...
        Column::Varchar(vec!["Platon", "Aristoteles", "Wittgenstein", "Rhetikus", "Newton", "Spinoza"]),
        Column::Varchar(vec!["Ideenlehre", "Syllogistik", "Sprachteorie", "Planetenbewegung", "Keplersche Gesetze", "Gott und Natur"]),
